...
ll a, b, dp[20][10][20][2];

ll solve(string &num, ll n, ll prev_digit, ll leading_zeros, ll tight_constraint){
    if(n==0)return 1;
    if(dp[n][prev_digit][leading_zeros][tight_constraint]!=-1)return dp[n][prev_digit][leading_zeros][tight_constraint];
    ll res=0, ub=tight_constraint?num[sz(num)-n]-'0':9;
    rep1(dig, 0, ub){
        if(dig==prev_digit){
            if(!(dig==0 && leading_zeros))continue;
        }
        res=(res + solve(num, n-1, dig, ((dig==0)&leading_zeros), (tight_constraint&(dig==ub))));
    }
    return dp[n][prev_digit][leading_zeros][tight_constraint] = res;
}
...
void solve(){   
    ...
    ll B=solve(str, sz(str), -1, 1, 1);
    memset(dp, -1, sizeof(dp));
    str=to_string(a-1);
    ll A=solve(str, sz(str), -1, 1, 1);
    ll res=(B - A);
    cout << res << endl;
}   
