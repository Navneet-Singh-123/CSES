...
// Create a vector of multisets storing the k shortest values for each node
...
void solve(){   
    ...
    while(!pq.empty()){
        auto it=pq.top();
        pq.pop();
        ll d=it.ff, node=it.ss;
        if(!vis[node])continue;
        vis[node]--;
        for(auto it: edges[node]){
            ll adj=it.ff, wt=it.ss;
            if(sz(dist[adj]) < k){
                dist[adj].insert(d+wt);
                pq.push({d+wt, adj});
            }
            else if(*dist[adj].rbegin() > d+wt){
                dist[adj].erase(prev(dist[adj].end()));
                dist[adj].insert(d+wt);
                pq.push({d+wt, adj});
            }
        }
