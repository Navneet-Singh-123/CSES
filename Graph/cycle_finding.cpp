...
vector<edge> edges;
vector<ll> cycle, dist(N, inf), relaxant(N, -1);
...
void bellman_ford(ll src){
    ...
    if(node==-1){
        // If node==-1 it means in the nth iteration there was no relaxation which means no negetive cycle
        return;
    }
    rep1(i, 1, n)node=relaxant[node];
    for(ll v=node;;v=relaxant[v]){
        cycle.push_back(v);
